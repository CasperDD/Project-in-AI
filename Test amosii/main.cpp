...
public:
                     ...
                     lpzrobots::GlobalData &global)
  {
    ...
    setCameraHomePos(
        ...
        lpzrobots::Pos(0.0, -5.0, 10.0),
        lpzrobots::Pos(0.0, 0.0, 0.0));

    // set simulation parameters
    global.odeConfig.setParam("controlinterval", 10);
    global.odeConfig.setParam("simstepsize", 0.01);
    global.odeConfig.setParam("noise", 0.03); // normal noise = 0.3 Reduce to 0.05 for Decoupled CPGs (PM)

    // add playground
    lpzrobots::Playground *playground = new lpzrobots::Playground(odeHandle, osgHandle,
                                                                  osg::Vec3(6.5, 0.2, 0.3));
    playground->setTexture(0, 0, lpzrobots::TextureDescr("Images/wall_bw.jpg", -1.5, -3));
    playground->setPosition(osg::Vec3(2.5, 0, .0));
    ...
    if (obstacle_active)
    {
      ...
      b1->setPose(osg::Matrix::rotate(/*-0.5*(M_PI/4)*/ (M_PI / 2), 0, 0, 1) * osg::Matrix::translate(2.5, 0.0, 0.0) /* pose*/);
    ...
    s1->setPosition(osg::Vec3(5, 0.0, 0.1)); // 0.1
    ...
    s2->setPosition(osg::Vec3(200.5, 0.0, 0.1)); // 0.1
    ...
    s3->setPosition(osg::Vec3(300.0, 1.5, 0.1)); // 0.1
    s3->setTexture("Images/dusty.rgb");
    s3->setColor(lpzrobots::Color(0, 1, 0));
    obst.push_back(s3);
    ...
    amos->place(osg::Matrix::translate(.0, .0, 0.0)); //0.5 for placing in air
