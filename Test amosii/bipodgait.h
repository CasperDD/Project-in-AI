...
public:
      ...
      : AbstractController("EmptyController", "$Id: tripodgait18dof.cpp,v 0.1 $")
  {
    ...
    saveFile1.open("bi_savedata1.txt", ios::out);
    saveFile2.open("bi_savedata2.txt", ios::out);
    saveFile3.open("bi_savedata3.txt", ios::out);
    saveFile4.open("bi_savedata4.txt", ios::out);
                              ...
                              motor *y_, int number_motors)
  {
    ...
    //----ICO learning-------//
    u_ico = w_ico * predictive_signal + reflexive_signal; // Red

    deri_xt_reflex_angle = reflexive_signal - reflex_old;

    // Weights of ICO learning modify these by implementing ICO learning rule!!

    w_ico += rate_ico * deri_xt_reflex_angle * predictive_signal; // Red

    // exp_output*exp(-w_ico)
    // Exponential decay of exploration
    exp_decay = exp_output * exp(-w_ico);

    u_ico_out = exp_decay + u_ico;
    ...
    /// Neural Control II: VRN implementation, for students ///
    // M0
    y_[TR0_m] = -WeightM0_H2 * outputH13; // outputH2 goes through VRN_right and output outputH13
    // M1
    y_[TR1_m] = WeightM1_H2 * outputH13; // outputH2 goes through VRN_right and output outputH13
    // M2
    y_[TR2_m] = -WeightM2_H2 * outputH13; // outputH2 goes through VRN_right and output outputH13

    // M3
    y_[TL0_m] = -WeightM3_H2 * outputH14; // outputH2 goes through VRN_left and output outputH14
    // M4
    y_[TL1_m] = WeightM4_H2 * outputH14; // outputH2 goes through VRN_left and output outputH14
    // M5
    y_[TL2_m] = -WeightM5_H2 * outputH14; // outputH2 goes through VRN_left and output outputH14
